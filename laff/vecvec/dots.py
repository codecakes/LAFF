...
def dots(x, y, alpha ):
    ...
    if isinstance(alpha,matrix): m_alpha, n_alpha = alpha.shape
    #(type(alpha) is matrix): m_alpha, n_alpha = alpha.shape
        
    assert (m_alpha == 1 and n_alpha == 1), \
           "laff.dots: alpha must be a 1 x 1 matrix"
        
    m_x, n_x = x.shape
    m_y, n_y = y.shape
    
    assert m_x == 1 or n_x == 1, "laff.dots: x is not a vector"
    assert m_y == 1 or n_y == 1, "laff.dots: y is not a vector"
